...
##### 4.2.3 为什么我们把一个 patch 与下一层的多个神经元相连呢？一个神经元不够好吗？
...
### 10.0 Inception 模块
Inception网络或Inception层的作用是:代替人工来确定卷积中的过滤类型，或者确定是否需要创建卷积层或池化层。

基本思想：Inception网络不需要人为决定使用哪个滤波器，或者是否需要池化，而是由网络自行确定这些参数。
