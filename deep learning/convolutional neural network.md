...
### 11.1 三个3×3卷积核比一个7×7卷积核更高效
![conv_2.png](https://i.imgur.com/3O8CZBs.png)

假设网络有三层3×3的卷积层，则第三层卷积层的单个神经元对输入的感受野范围为7×7，这正好对应一层7×7大小的卷积层，因为他们看到的是同样大小的输入区域。

假设输入shape为H×W×C，卷积核数量为C (步长为1，输出与输入形状相同)，比较7×7与3×3的学习参数数量以及计算量 (不考虑偏置项)：
...
乘加计算数量：</br>
7x7:49HWC2 ((H×W×C)×(7×7×C))</br>
3x3:27HWC2 (3×(H×W×C)×(3×3×C))

****综合考虑****每层卷积层后的ReLU激活函数，后者比前者的**参数、计算量更少、更加非线性**。</br>
由上看出卷积核越小越好，但不能全设置为1×1，否则就计算不到感受野了。
### 11.2 多层瓶颈结构
![conv_3.png](https://i.imgur.com/TugSLvX.png)

计算量：((H×W×C)×(1×1×1/2xC)) + ((H×W×1/2xC)×(3×3×1/2C)) + ((H×W×C)×(1×1×1/2xC))

三个3×3卷积核和多层瓶颈结构相比，后者比前者的**参数、计算量更少、更加非线性**。</br>
### 11.3 N×N 核分解成不对称的1×N和N×1核
![conv_4.png](https://i.imgur.com/zZOwEJG.png)
