...
### 1.0 支持度的计算
#### 1.1.1 原理
设数据集`(D)：[set([1, 3, 4]), set([2, 3, 5]), set([1, 2, 3, 5]), set([2, 5])]`；
候选项集列表`(C1)：[frozenset([1]), frozenset([2]), frozenset([3]), frozenset([4]), frozenset([5])]`；
`frozenset([1])`在`D`中出现的次数为2，
同理其他：`frozenset([4]): 1, frozenset([5]): 3, frozenset([2]): 3, frozenset([3]): 3`
